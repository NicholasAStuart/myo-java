...
public final class Hub {
	...
	private boolean copyAndLoadOSXFromTemp() {
		try {
			...
			InputStream macMyoInputStream = this.getClass().getResourceAsStream("/osx/libmyo.jnilib");
			...
			InputStream macMyoLibInputStream = this.getClass().getResourceAsStream("/osx/myo.framework/Versions/A/myo");
	...
	private boolean copyAndLoadX64FromTemp() {
		try {
			...
			InputStream myo64DllInputStream = this.getClass().getResourceAsStream("/x64/myo64.dll");
			...
			InputStream jniJavaMyoLibDllInputStream = this.getClass().getResourceAsStream("/x64/JNIJavaMyoLib.dll");
	...
	private boolean copyAndLoadWin32FromTemp() {
		try {
			...
			InputStream myo32DllInputStream = this.getClass().getResourceAsStream("/Win32/myo32.dll");
			...
			InputStream jniJavaMyoLibDllInputStream = this.getClass().getResourceAsStream("/Win32/JNIJavaMyoLib.dll");
			Files.copy(jniJavaMyoLibDllInputStream, jniJavaMyoLibDllTempFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
